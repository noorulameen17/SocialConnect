...
export default function PasswordResetCompletePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!code) { setErr('Missing code'); setReady(true); return; }
      const { error } = await supabase.auth.exchangeCodeForSession(code);
      if (error) setErr(error.message || 'Link invalid or expired');
      setReady(true);
    })();
  }, [supabase]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) setErr(error.message); else { setMsg('Password updated'); setTimeout(()=> router.push('/auth/login'), 1200); }
