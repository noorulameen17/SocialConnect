...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if (acting.id === params.id) return NextResponse.json({ error: 'Cannot modify own admin status' }, { status: 400 });
  ...
  const { data, error } = await supabase.from('profiles').update(updates).eq('id', params.id).select('id, is_admin, active').single();
